...
public class DetectionManager {
  // this is a list which is bad for performance since we are looping through it multiple times a
  // tick. I believe this is ok?? we should change hte data structe if we notice problems
  // maybe need to add cleanup to put objects close to each other so we don't run into two different
  ...
  public Pose2d getClosestObjectPosition(Pose2d robotPose) {
    ...
    // returns the robot pose to stop a robot from accidentally piding accross the field and hurting
    // someone
    if (bestIndex == -1) {
      return robotPose;
    }
